...
export default class TableGenerator {
    ...
    async generateTable(allRepoScores, output, options) {
        const promises = Array.from(allRepoScores).map(async ([repoName, repoScores]) => {
            ...
            // 순위 계산 함수
            /**
             * 현재 참가자의 점수를 기준으로 순위를 계산
             * 이전 참가자와 점수가 같으면 같은 순위를 유지하고,
             * 다르면 현재 인덱스에 +1을 더한 새로운 순위를 반환
             * @param {number} currentIndex - 현재 참가자의 인덱스 (0부터 시작)
             * @param {number} currentTotal - 현재 참가자의 총 점수
             * @param {number} prevTotal - 직전 참가자의 총 점수
             * @param {number} prevRank - 직전 참가자의 순위
             * @returns {number} - 계산된 현재 참가자의 순위
             */
            ...
            // 참여율 계산 함수
            /**
             * 주어진 점수의 비율(%)을 계산하여 소수점 둘째 자리까지 문자열로 반환
             * 전체 점수가 0이면 '0.00'을 반환
             * @param {number} score - 개별 점수 또는 획득한 점수
             * @param {number} totalScore - 전체 기준 점수
             * @returns {string} - 소수점 둘째 자리까지 포맷된 백분율 문자열
             */
            ...
            /**
             * 밝은 계열의 RGB 색상을 랜덤으로 생성하여 ANSI 이스케이프 코드 문자열을 반환
             * 터미널 출력에 컬러를 적용할 때 사용
             * @returns {string} - RGB ANSI 이스케이프 코드 문자열 ex) '\x1b[38;2;123;200;255m'
             */
